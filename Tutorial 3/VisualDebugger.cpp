...
namespace VisualDebugger
{
	...
	void HUDInit()
	{
		...
		hud.AddLine(HELP, "");
		hud.AddLine(HELP, " Move the ball");
		hud.AddLine(HELP, "    I,K,J,L, - Forward,Backward,Left,Right");
		hud.AddLine(HELP, "");
		hud.AddLine(HELP, " Shoot Objects");
		hud.AddLine(HELP, "    O : Shoot box");
		hud.AddLine(HELP, "    P: Shoot Sphere");
		hud.AddLine(HELP, "");
		hud.AddLine(HELP, " Press (X) to release Hammer");
	...
	void ballInput(int key)
	{
		...
		case 'O': //right
			...
			scene->shoot->Shoot(camera->getEye(), camera->getDir());
			scene->shoot->AddToScene(scene);
			

			break;
		case 'P': //right

			scene->shoot2->Shoot(camera->getEye(), camera->getDir());
			scene->shoot2->AddToScene(scene);
